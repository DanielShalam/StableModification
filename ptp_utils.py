...
def init_latent(latent, model, height=256, width=256, generator=None, batch_size=1):
    if latent is None:
...
def text2image_ldm_v2(
    ...
    uncond_embeddings = text_encoder(uncond_input.input_ids.to(model.device))[0]

    text_input = tokenizer(prompt, padding="max_length", max_length=77, return_tensors="pt")
    text_embeddings = text_encoder(text_input.input_ids.to(model.device))[0]
    latent, latents = init_latent(latent, model, height, width, generator, batch_size)
    context = torch.cat([uncond_embeddings, text_embeddings])
...
class AttentionReweight(AttentionControlEdit):
    ...
    def __init__(self, tokenizer, prompts, num_steps: int, cross_replace_steps: float, self_replace_steps: float, equalizer,
        ...
        super(AttentionReweight, self).__init__(tokenizer, prompts, num_steps, cross_replace_steps, self_replace_steps,
                                                local_blend)
...
def show_cross_attention(tokenizer, prompts, attention_store: AttentionStore, res: int, from_where: List[str],
                         select: int = 0):
...
def run_and_display(model, prompts, controller, num_steps=50, guidance_scale=7.5, latent=None, run_baseline=False,
                    generator=None, outdir: str = None):
    if run_baseline:
        print("w.o. prompt-to-prompt")
        images, latent = run_and_display(model, prompts, EmptyControl(), num_steps=num_steps,
                                         guidance_scale=guidance_scale, latent=latent, run_baseline=False,
                                         generator=generator, outdir=outdir)
        print("with prompt-to-prompt")
    images, x_t = text2image_ldm_stable(model, prompts, controller, num_inference_steps=num_steps,
                                        guidance_scale=guidance_scale, latent=latent,
                                        generator=generator, low_resource=False)

    # base_count = len(os.listdir(outdir)) + 1
    # img = view_images(images)
